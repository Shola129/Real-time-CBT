  ...
  if(dep==="" || subject===""){
    ...
    if(result.status==="success"){
      ...
      for (let index = 0; index < response.length; index++) {
        ...
        document.getElementById("editQ-session").value=response[index].session;
        document.getElementById("editQ-level").value=response[index].level;
        document.getElementById("editQ-semster").value=response[index].semster;
      }
...
document.getElementById("editQuestionSaveBtn").addEventListener("click", async function(){
      ...
      const optionE = document.getElementById("editQ-optionE").value;
      const correctAss = document.getElementById("editQ-correctAss").value;
      const questionID = document.getElementById("editQ-ID").value;
      const session = document.getElementById("editQ-session").value;
      const level = document.getElementById("editQ-level").value;
      const semster = document.getElementById("editQ-semster").value;
      if (!dept || !subj || !questionText || !optionA || !optionB || !optionC || !optionD || !optionE || !correctAss || !questionID || !level ||! semster||!session) {
        alert("All field require");
        return;
      } else {
...
async function processQuestionData(){
      event.preventDefault();
      const dep = document.getElementById("displayDept").textContent.trim();
      const sub = document.getElementById("displaySubject").textContent.trim();
      const session = document.getElementById("displaySession").textContent.trim();
      const level = document.getElementById("displayLevel").textContent.trim();
      const semster = document.getElementById("displaySemster").textContent.trim();
      const org_code = document.getElementById("organization-code").textContent;
      ...
      if(result.status==="success"){
        ...
        alert("Error occur at our end, try again later");
      }
  }
...
document.getElementById('nextToQuestionBtn')?.addEventListener('click', () => {
  ...
  const session = document.getElementById('questionSession').value.trim();
  const level = document.getElementById('questionLevel').value.trim();
  const semster = document.getElementById('questionSemester').value.trim();
  if(!dept||!subj || !session || !level || !semster){ alert('Complete fields'); return; }
  ...
  document.getElementById('displaySemster').textContent=semster;
